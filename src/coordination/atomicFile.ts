 ...
 * with `MOVEFILE_REPLACE_EXISTING` on Windows, so a window reading the shared
 * snapshot never observes a half-written file.
 *
...
async function renameWithRetry(from: string, to: string): Promise<void> {
    for (let attempt = 0; ; attempt++) {
        try {
            ...
            if (attempt >= RENAME_RETRY_DELAYS_MS.length || !isContendedRename(error)) {
                throw error;
            }
...
function isContendedRename(error: unknown): boolean {
