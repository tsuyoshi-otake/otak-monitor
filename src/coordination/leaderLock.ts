...
/**
 * How long a lease survives without a heartbeat before another window may take
 * it. Three missed heartbeats, so an ordinary hiccup — a slow disk, a laptop
 * that just woke, an extension host busy with a long directory walk — never
 * costs the leader its lease, while a killed window is replaced within half a
 * minute.
 */
...
export function isLockRecord(raw: unknown): raw is LockRecord {
    const record = raw as LockRecord | undefined;
    return !!record && typeof record === 'object' &&
        record.version === LOCK_VERSION &&
        typeof record.holder === 'string' && record.holder !== '' &&
        typeof record.heartbeatMs === 'number' && Number.isFinite(record.heartbeatMs);
}
...
/**
 * A lease over one sampling scope, shared through a file that every VS Code
 * window of the same installation can see. Exactly one window holds it at a
 * time and does the sampling; the rest read the snapshot it publishes.
 *
...
export class LeaderLock {
     ...
     * window — the caller must then stop sampling and go back to following.
     */
